    ...
    def ready(self):
        ...
        def preload_in_background():
            try:
                ...
                logger.info(
                    "Preloading summarization models in background (this may take 30-60 seconds)..."
                )
                logger.info("Django server is starting - models will be ready shortly")
                preload_summarizer()
                logger.info(
                    "✓ Summarization models preloaded successfully - ready for requests"
                )
            except Exception as e:
                logger.error(
                    f"✗ Failed to preload summarization models: {e}", exc_info=True
                )
        ...
        thread.start()
        logger.info(
            "Summarizer preload started in background - Django server starting immediately"
        )
