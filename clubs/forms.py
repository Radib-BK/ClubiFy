...
class ClubForm(forms.ModelForm):
    color = forms.ChoiceField(
        required=False,
        choices=[("", "Random pastel (default)")]
        + [(color, color) for color in PASTEL_COLORS],
    ...
    logo = forms.FileField(
        ...
        widget=ClearableFileInput(attrs={"class": "input", "accept": "image/*,.svg"}),
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "input", "placeholder": "Enter club name"}
            ),
